...
sGis.module('Bbox', ['utils', 'CRS', 'Point'], function(utils, CRS, Point) {
    ...
    class Bbox {
        ...
        intersect(bbox) {
            ...
            return new Bbox(
                [Math.min(this.xMin, bbox.xMin), Math.min(this.yMin, bbox.yMin)],
                [Math.max(this.xMax, bbox.xMax), Math.max(this.yMax, bbox.yMax)],
                this.crs,
            );
        }
        ...
        intersects(bbox) {
            ...
            return (
                this.xMax > projected.xMin &&
                this.xMin < projected.xMax &&
                this.yMax > projected.yMin &&
                this.yMin < projected.yMax
            );
        }
        ...
        contains(point) {
            ...
            return (
                this.xMin <= projected.x &&
                this.xMax >= projected.x &&
                this.yMin <= projected.y &&
                this.yMax >= projected.y
            );
        }
...
sGis.module('CRS', ['Crs', 'math'], function(Crs, math) {
            ...
            let Y =
                ...
                Math.log(
                    Math.tan(Math.PI / 4 + rLat / 2) *
                        Math.pow((1 - e * Math.sin(rLat)) / (1 + e * Math.sin(rLat)), e / 2),
                );
    ...
    CRS.moscowBessel = new Crs({
        wkt:
            'PROJCS["Moscow_bessel",GEOGCS["GCS_Bessel_1841",DATUM["D_Bessel_1841",SPHEROID["Bessel_1841",6377397.155,299.1528128]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Transverse_Mercator"],PARAMETER["False_Easting",0.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",37.5],PARAMETER["Scale_Factor",1.0],PARAMETER["Latitude_Of_Origin",55.66666666666666],UNIT["Meter",1.0]]',
    });
...
sGis.module('DynamicLayer', ['utils', 'Layer', 'feature.Image', 'symbol.image.Image'], function(
    utils,
    /** sGis.Layer.constructor */ Layer,
    Image,
    ImageSymbol,
) {
    'use strict';
    ...
    class DynamicLayer extends Layer {
        ...
        getFeatures(bbox, resolution) {
            ...
            if (
                this._forceUpdate ||
                !this._features[0].bbox.equals(bbox) ||
                this._features[0].width !== width ||
                this._features[0].height !== height
            ) {
        ...
        _createFeature(bbox) {
            var feature = new Image(bbox, {
                crs: this.crs || bbox.crs,
        ...
        _updateSymbol() {
            if (this._features)
                this._features[0].symbol = new ImageSymbol({
                    opacity: this.opacity,
                });
        }
    }
...
sGis.module('EventHandler', ['utils'], function(utils) {
    ...
    class EventHandler {
        ...
        forwardEvent(sGisEvent) {
            if (
                this._prohibitedEvents &&
                this._prohibitedEvents.indexOf(sGisEvent.eventType) !== -1
            )
                return;
        ...
        fire(eventType, parameters) {
            if (this._prohibitedEvents && this._prohibitedEvents.indexOf(eventType) !== -1)
                return null;
            ...
            if (types.length !== 1)
                utils.error(
                    'Exactly on type of event can be fired at a time, but ' +
                        types.length +
                        ' is given',
                );
        ...
        addListener(description, handler) {
            if (!(handler instanceof Function))
                utils.error('Function is expected but got ' + handler + ' instead');
            if (!utils.isString(description))
                utils.error('String is expected but got ' + description + ' instead');
            ...
            for (var i = 0; i < types.length; i++) {
                ...
                this._eventHandlers[types[i]].push({
                    handler: handler,
        ...
        once(description, handler) {
            ...
            var types = getTypes(description);
            if (types.length !== 1)
                utils.error('Only one event type can be specified with .once() method');
            var namespaces = getNamespaces(description);
            if (!this._eventHandlers) this._setHandlerList();

            if (!this._eventHandlers) this._setHandlerList();
            if (!this._eventHandlers[types[0]]) this._eventHandlers[types[0]] = [];
            this._eventHandlers[types[0]].push({
                handler: handler,
                namespaces: namespaces,
                oneTime: true,
            });
        ...
        removeListener(description, handler) {
            if (!utils.isString(description))
                utils.error(
                    'Expected the name of the event and handler function, but got (' +
                        description +
                        ', ' +
                        handler +
                        ') instead',
            ...
            if (namespaces.length === 0) {
                if (types.length === 0)
                    utils.error('At least one event type or namespace must be specified');
                if (!handler)
                    utils.error(
                        'To remove all listeners of the given type use the .removeAllListeners() method',
                    );
            }
                        ...
                        if (
                            (namespaces === null ||
                                ...
                                utils.arrayIntersect(
                                    this._eventHandlers[types[i]][j].namespaces,
                                    namespaces,
                                )) &&
        ...
        hasListener(type, handler) {
            if (!utils.isString(type) || !utils.isFunction(handler))
                utils.error(
                    'Expected the name of the event and handler function, but got (' +
                        type +
                        ', ' +
        ...
        hasListeners(description) {
            if (!utils.isString(description))
                utils.error('Expected the name of the event, but got ' + description + ' instead');
        ...
        getHandlers(type) {
            if (!utils.isString(type))
                utils.error('Expected the name of the e*vent, but got ' + type + ' instead');
            if (this._eventHandlers && this._eventHandlers[type]) {
...
sGis.module('geotools', ['math', 'utils', 'CRS', 'Point'], function(math, utils, CRS, Point) {
    ...
    geotools.length = function(rings, crs, enclose = false) {
        ...
        for (let ring = 0, l = rings.length; ring < l; ring++) {
            ...
            for (let i = 0, m = ringTemp.length - 1; i < m; i++) {
                length += geotools.distance(
                    new Point(ringTemp[i], crs),
                    new Point(ringTemp[i + 1], crs),
                );
            }
        }
                    ...
                    if (!(point[1] === points[i][1] && point[1] === points[i - 1][1])) {
                        ...
                        if (
                            geotools.intersects(
                                ...
                                [point, [Math.max(points[i][0], points[i - 1][0]), point[1]]],
                            )
                        ) {
                            intersectionCount++;
                        }
        ...
        for (let ring = 0; ring < coordinates.length; ring++) {
            for (let i = 0; i < coordinates[ring].length; i++) {
                if (
                    ring === exc[0] &&
                    (Math.abs(i - exc[1]) < 2 ||
                        (exc[1] === 0 && i === coordinates[ring].length - 1) ||
                        (i === 0 && exc[1] === coordinates[ring].length - 1))
                )
                    continue;

                if (
                    intersects(
                        [coordinates[ring][i], coordinates[ring][i + 1] || coordinates[ring][0]],
                        line,
                    )
                )
                    return true;
    ...
    geotools.move = function(features, translate, mapCrs = null) {
        geotools.transform(
            features,
            [[1, 0, 0], [0, 1, 1], [translate[0], translate[1], 1]],
            [0, 0],
            mapCrs,
        );
    };
...
sGis.module('init', ['sGis', 'Map', 'painter.DomPainter'], (sGis, Map, Painter) => {
    'use strict';
...
sGis.module('Layer', ['utils', 'EventHandler'], function(utils, EventHandler) {
    ...
    class Layer extends EventHandler {
        ...
        checkVisibility(resolution) {
            return (
                this._isDisplayed &&
                (this.resolutionLimits[0] < 0 || resolution >= this.resolutionLimits[0]) &&
                (this.resolutionLimits[1] < 0 || resolution <= this.resolutionLimits[1])
            );
        }
...
sGis.module('LayerGroup', ['utils', 'EventHandler'], function(utils, EventHandler) {
    ...
    class LayerGroup extends EventHandler {
        ...
        constructor(layers = []) {
            super();
            this._layers = [];
            this._forwardEvent = sGisEvent => {
                this.fire(sGisEvent.eventType, sGisEvent);
            };
        ...
        _setForwardListeners(layerGroup) {
            layerGroup.on(
                'layerAdd layerRemove layerOrderChange contentsChange',
                this._forwardEvent,
            );
        }
        ...
        contains(layer) {
            for (var i = 0, l = this._layers.length; i < l; i++) {
                if (
                    (this._layers[i] instanceof LayerGroup && this._layers[i].contains(layer)) ||
...
sGis.module('Map', ['utils', 'CRS', 'Point', 'Bbox', 'LayerGroup', 'TileScheme'], function(
    utils,
    CRS,
    Point,
    Bbox,
    ...
    class Map extends LayerGroup {
        ...
        zoom(k, basePoint) {
            ...
            if (tileScheme) {
                ...
                resolution = tileScheme.levels[level]
                    ? tileScheme.levels[level].resolution
                    : currResolution;
            } else {
            ...
            resolution = Math.min(
                Math.max(resolution, this.minResolution || 0),
        ...
        _getScaledPosition(newResolution, basePoint) {
            ...
            return new Point(
                [
                    (position.x - basePoint.x) * scalingK + basePoint.x,
                    (position.y - basePoint.y) * scalingK + basePoint.y,
                ],
                position.crs,
            );
        }
        ...
        setResolution(resolution, basePoint, doNotAdjust) {
            this.setPosition(
                this._getScaledPosition(resolution, basePoint),
                doNotAdjust ? resolution : this.getAdjustedResolution(resolution),
            );
        }
        ...
        set minResolution(/** Number */ resolution) {
            if (resolution !== null) {
                ...
                if (resolution > maxResolution)
                    utils.error('maxResolution cannot be less then minResolution');
            }
...
sGis.module('Point', ['utils', 'CRS'], function(/**sGis.utils*/ utils, /**sGis.CRS*/ CRS) {
    'use strict';
    ...
    class Point {
        ...
        projectTo(newCrs) {
            ...
            if (projection === null)
                utils.error('Cannot project point to crs: ' + newCrs.stringDescription);
        ...
        equals(point) {
            return (
                utils.softEquals(point.x, this.x) &&
                utils.softEquals(point.y, this.y) &&
                point.crs.equals(this.crs)
            );
        }
...
sGis.module(
    'TileLayer',
    ['utils', 'TileScheme', 'Layer', 'Point', 'Bbox', 'feature.Image', 'CRS', 'symbol.image.Image'],
    function(utils, TileScheme, Layer, Point, Bbox, ImageF, CRS, ImageSymbol) {
        ...
        class TileLayer extends Layer {
            ...
            getFeatures(bbox, resolution) {
                ...
                let xStartIndex = Math.floor(
                    (trimmedBbox.xMin - this.tileScheme.origin[0]) /
                        this.tileWidth /
                        layerResolution,
                );
                let xEndIndex = Math.ceil(
                    (trimmedBbox.xMax - this.tileScheme.origin[0]) /
                        this.tileWidth /
                        layerResolution,
                );
                ...
                if (this.tileScheme.reversedY) {
                    yStartIndex = Math.floor(
                        (trimmedBbox.yMin - this.tileScheme.origin[0]) /
                            this.tileHeight /
                            layerResolution,
                    );
                    yEndIndex = Math.ceil(
                        (trimmedBbox.yMax - this.tileScheme.origin[0]) /
                            this.tileHeight /
                            layerResolution,
                    );
                } else {
                    yStartIndex = Math.floor(
                        (this.tileScheme.origin[1] - trimmedBbox.yMax) /
                            this.tileHeight /
                            layerResolution,
                    );
                    yEndIndex = Math.ceil(
                        (this.tileScheme.origin[1] - trimmedBbox.yMin) /
                            this.tileHeight /
                            layerResolution,
                    );
                }
                ...
                for (var xIndex = xStartIndex; xIndex < xEndIndex; xIndex++) {
                    ...
                    for (var yIndex = yStartIndex; yIndex < yEndIndex; yIndex++) {
                        var yIndexAdj = this.cycleY
                            ? this._getAdjustedIndex(yIndex, level)
                            : yIndex;
                        var tileId = TileLayer.getTileId(
                            this.tileScheme.levels[level].zIndex,
                            xIndex,
                            yIndex,
                        );
                        ...
                        if (!tiles[tileId]) {
                            ...
                            var tileUrl = this.getTileUrl(
                                xIndexAdj,
                                yIndexAdj,
                                this.tileScheme.levels[level].zIndex,
                            );
                            tiles[tileId] = new ImageF(imageBbox, {
                                src: tileUrl,
                                symbol: this._symbol,
                                crs: this.crs,
                            });
            ...
            _getTileBbox(level, xIndex, yIndex) {
                ...
                let minY = this.tileScheme.reversedY
                    ? yIndex * this.tileHeight * resolution + this.tileScheme.origin[1]
                    : -(yIndex + 1) * this.tileHeight * resolution + this.tileScheme.origin[1];
                let startPoint = new Point(
                    [xIndex * this.tileWidth * resolution + this.tileScheme.origin[0], minY],
                    this.crs,
                );

                let maxY = this.tileScheme.reversedY
                    ? (yIndex + 1) * this.tileHeight * resolution + this.tileScheme.origin[1]
                    : -yIndex * this.tileHeight * resolution + this.tileScheme.origin[1];
                let endPoint = new Point(
                    [(xIndex + 1) * this.tileWidth * resolution + this.tileScheme.origin[0], maxY],
                    this.crs,
                );
    ...
    class TileScheme {
        ...
        getLevel(resolution, direction = false) {
            if (!this.levels || this.levels.length === 0)
                utils.error('Tile scheme levels are not set');
...
sGis.module('controls.Circle', ['controls.PolyDrag', 'feature.Polygon'], function(
    PolyDrag,
    Polygon,
) {
    'use strict';
    ...
    class Circle extends PolyDrag {
        _startNewFeature(point) {
            ...
            this._activeFeature = new Polygon([[]], {
                crs: point.crs,
                symbol: this.symbol,
            });
        ...
        _updateFeature(point) {
            let radius = Math.sqrt(
                Math.pow(this._centerPoint[0] - point.position[0], 2) +
                    Math.pow(this._centerPoint[1] - point.position[1], 2),
            );
...
sGis.module('Control', ['utils', 'EventHandler'], function(
    /** sGis.utils */ utils,
    ...
    class Control extends EventHandler {
        ...
        set activeLayer(/** sGis.FeatureLayer */ layer) {
    ...
    function(
        ...
        class Editor extends Control {
            ...
            _setEditors() {
                ...
                this._polyEditor = new PolyEditor(this.map, {
                    onFeatureRemove: this._delete.bind(this),
                });
            ...
            _saveState() {
                this._states.setState({
                    ...
                    coordinates: this._activeFeature && this._activeFeature.coordinates,
                });
            }
            ...
            _setState(state) {
                ...
                if (!state.coordinates && this.activeLayer.features.indexOf(state.feature) >= 0) {
                    ...
                    state.coordinates &&
                    this.activeLayer.features.indexOf(state.feature) < 0
                ) {
...
sGis.module(
    'controls.MultiPoint',
    ['utils', 'Control', 'FeatureLayer', 'symbol.point.Point', 'feature.MultiPoint'],
    function(utils, Control, FeatureLayer, PointSymbol, MultiPointFeature) {
        ...
        class MultiPoint extends Control {
            ...
            startNewFeature(point) {
                ...
                this._activeFeature = new MultiPointFeature([point.position], {
                    crs: this.map.crs,
                    symbol: this.symbol,
                });
...
sGis.module('controls.Point', ['Control', 'feature.Point', 'symbol.point.Point'], function(
    ...
    class PointControl extends Control {
        ...
        _handleClick(sGisEvent) {
            ...
            var feature = new PointFeature(point.position, {
                crs: this.map.crs,
                symbol: this.symbol,
            });
...
sGis.module('controls.PointEditor', ['Control', 'controls.Snapping'], (Control, Snapping) => {
    'use strict';
    ...
    class PointEditor extends Control {
        ...
        _handleDrag(sGisEvent) {
            this._activeFeature.position =
                this._snapping.position ||
                sGisEvent.point.projectTo(this._activeFeature.crs).position;
...
sGis.module('controls.Poly', ['utils', 'Control', 'FeatureLayer'], function(
    utils,
    Control,
    FeatureLayer,
) {
    ...
    class PolyControl extends Control {
            ...
            setTimeout(() => {
                ...
                if (this._activeFeature) {
                    if (sGisEvent.ctrlKey) {
                        ...
                        this._activeFeature.addPoint(
                            sGisEvent.point,
                            this._activeFeature.rings.length - 1,
                        );
                    }
                } else {
                    this.startNewFeature(sGisEvent.point);
        ...
        startNewFeature(point) {
            ...
            this._activeFeature = this._getNewFeature([point.position, point.position], {
                crs: this.map.crs,
                symbol: this.symbol,
            });
            this._tempLayer.add(this._activeFeature);
...
sGis.module('controls.PolyDrag', ['Control', 'FeatureLayer', 'symbol.polygon.Simple'], function(
    Control,
    FeatureLayer,
    PolygonSymbol,
) {
    ...
    (Control, Snapping, geotools, PointSymbol, FeatureLayer, Point) => {
        ...
        class PolyEditor extends Control {
            ...
            _handleMousemove(sGisEvent) {
                if (
                    this.ignoreEvents ||
                    !this.vertexChangeAllowed ||
                    this._activeRing !== null ||
                    this._activeIndex !== null ||
                    sGisEvent.eventType === 'mouseout'
                ) {
                    this._tempLayer.features = [];
                }
                ...
                let firstPoint = this._getProjectedPoint(
                    ring[activeIndex],
                    this._activeFeature.crs,
                );
            ...
            _handleDragStart(sGisEvent) {
                ...
                if (Array.isArray(intersection) && this.vertexChangeAllowed) {
                    ...
                    let point = this._getProjectedPoint(
                        ring[intersection[1]],
                        this._activeFeature.crs,
                    );
                    let evPoint = sGisEvent.point.position;
                    ...
                    if (currDist < targetDist) {
                        ...
                        if (nextDist < targetDist) {
                            ...
                            this._activeFeature.insertPoint(
                                intersection[0],
                                intersection[1] + 1,
                                evPoint,
                            );
            ...
            _handleDrag(sGisEvent) {
                ...
                if (this.activeLayer) this.activeLayer.redraw();
                this.fire('change', {
                    ringIndex: this._activeRing,
                    pointIndex: this._activeIndex,
                });
            }
            ...
            _handleFeatureDrag(sGisEvent) {
                geotools.move(
                    [this._activeFeature],
                    [-sGisEvent.offset.x, -sGisEvent.offset.y],
                    this.map.crs,
                );
                this._activeFeature.redraw();
                if (this.activeLayer) this.activeLayer.redraw();
...
sGis.module(
    'controls.Polygon',
    ['controls.Poly', 'feature.Polygon', 'symbol.polygon.Simple'],
...
sGis.module(
    'controls.Polyline',
    ['controls.Poly', 'feature.Polyline', 'symbol.polyline.Simple'],
...
sGis.module(
    ...
    (
        ...
        class PolyTransform extends Control {
            ...
            _setRotationHandle() {
                this._rotationHandle = new PointFeature([0, 0], {
                    crs: this.map.crs,
                    symbol: this.rotationHandleSymbol,
                });
            ...
            _setScaleHandles() {
                ...
                for (let i = 0; i < 9; i++) {
                    ...
                    symbol.offset = {
                        x: this.scaleHandleOffset * xk,
                        y: this.scaleHandleOffset * yk,
                    };

                    this._scaleHandles[i] = new PointFeature([0, 0], {
                        symbol: symbol,
                        crs: this.map.crs,
                    });
            ...
            _handleRotationStart(sGisEvent) {
                ...
                this._rotationBase = this._activeFeature.bbox.center.projectTo(
                    this.map.crs,
                ).position;
            ...
            _handleRotation(sGisEvent) {
                ...
                let alpha1 =
                    xPrev === this._rotationBase[0]
                        ? Math.PI / 2
                        : Math.atan2(yPrev - this._rotationBase[1], xPrev - this._rotationBase[0]);
                let alpha2 =
                    sGisEvent.point.x === this._rotationBase[0]
                        ? Math.PI / 2
                        : Math.atan2(
                              sGisEvent.point.y - this._rotationBase[1],
                              sGisEvent.point.x - this._rotationBase[0],
                          );
		 ...
		 */
        PolyTransform.prototype.rotationHandleSymbol = new PointSymbol({
            offset: { x: 0, y: -30 },
        });
...
sGis.module('controls.Rectangle', ['controls.PolyDrag', 'feature.Polygon'], function(
    PolyDrag,
    Polygon,
) {
    'use strict';
    ...
    class Rectangle extends PolyDrag {
        _startNewFeature(point) {
            ...
            this._activeFeature = new Polygon([[position, position, position, position]], {
                crs: point.crs,
                symbol: this.symbol,
            });
            this.tempLayer.add(this._activeFeature);
        ...
        _updateFeature(point) {
            ...
            coord = [
                [coord[0], [coord[1][0], pointCoord[1]], pointCoord, [pointCoord[0], coord[3][1]]],
            ];
...
sGis.module(
    'controls.Snapping',
    ['Control', 'FeatureLayer', 'feature.Point', 'symbol.point.Point', 'Bbox', 'geotools'],
    (
        /** function(new:sGis.Control) */ Control,
        /** function(new:sGis.FeatureLayer) */ FeatureLayer,
        /** function(new:sGis.feature.Point) */ PointFeature,
        ...
        class Snapping extends Control {
            ...
            _onMouseMove(sGisEvent) {
                ...
                this._tempLayer.features = snapping
                    ? [
                          new PointFeature(snapping.position, {
                              crs: point.crs,
                              symbol: this.symbol,
                          }),
                      ]
            ...
            getSnapping(point) {
                ...
                for (var i = 0; i < this.snappingTypes.length; i++) {
                    let snappingResult = snapping[this.snappingTypes[i]](
                        point,
                        this.activeLayer,
                        distance,
                        this.activeFeature,
                        this.activeRingIndex,
		 ...
		 */
        Snapping.prototype.symbol = new PointSymbol({
            fillColor: 'red',
            size: 5,
        });
        ...
        var snapping = {
            vertex: function(point, layer, distance, activeFeature, activeRing, activeIndex) {
                ...
                for (let i = 0; i < features.length; i++) {
                    let feature = features[i].crs.equals(point.crs)
                        ? features[i]
                        : features[i].projectTo(point.crs);

                    if (feature.position) {
                        if (features[i] === activeFeature) continue;
                        if (
                            Math.abs(feature.x - point.x) < distance &&
                            Math.abs(feature.y - point.y) < distance
                        ) {
                            return {
                        ...
                        for (let ring = 0; ring < rings.length; ring++) {
                            for (let j = 0; j < rings[ring].length; j++) {
                                if (
                                    features[i] === activeFeature &&
                                    ring === activeRing &&
                                    (Math.abs(j - activeIndex) < 2 ||
                                        Math.abs(j - activeIndex) === rings[ring].length - 1)
                                )
                                    continue;

                                if (
                                    Math.abs(rings[ring][j][0] - point.x) < distance &&
                                    Math.abs(rings[ring][j][1] - point.y) < distance
                                ) {
                                    return {
                                        position: rings[ring][j],
                                        feature: features[i],
                                        ring: ring,
                                        index: j,
                                    };
            ...
            midpoint: function(point, layer, distance, activeFeature, activeRing, activeIndex) {
                ...
                for (let i = 0; i < features.length; i++) {
                    ...
                    for (let ringIndex = 0; ringIndex < rings.length; ringIndex++) {
                        let ring = feature.isEnclosed
                            ? rings[ringIndex].concat([rings[ringIndex][0]])
                            : rings[ringIndex];

                        for (let j = 1; j < ring.length; j++) {
                            if (
                                features[i] === activeFeature &&
                                ringIndex === activeRing &&
                                (j === activeIndex ||
                                    j - 1 === activeIndex ||
                                    (activeIndex === 0 && j === ring.length - 1))
                            )
                                continue;
                            ...
                            if (
                                Math.abs(midPointX - point.x) < distance &&
                                Math.abs(midPointY - point.y) < distance
                            ) {
                                return {
                ...
                for (let i = 0; i < features.length; i++) {
                    ...
                    for (let ringIndex = 0; ringIndex < rings.length; ringIndex++) {
                        ...
                        for (let j = 1; j < ring.length; j++) {
                            ...
                            let projection = geotools.pointToLineProjection(point.position, [
                                ring[j - 1],
                                ring[j],
                            ]);
                            ...
                            if (
                                projection[0] >= minX &&
                                projection[0] <= maxX &&
                                Math.abs(projection[0] - point.x) < distance &&
                                Math.abs(projection[1] - point.y) < distance
                            ) {
                                return {
                                    position: projection,
                                    feature: features[i],
            ...
            axis: function(
                ...
                if (basePoint.length > 0) {
                    let position = [
                        basePoint[0] === undefined ? point.x : basePoint[0],
                        basePoint[1] === undefined ? point.y : basePoint[1],
                    ];
                    return {
                        position: position,
            ...
            orthogonal: function(
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (Math.abs(dx) < distance && Math.abs(dy) < distance) {
                        ...
                        let nextPoint = n
                            ? ring[(n + activeIndex + direction) % n]
                            : ring[activeIndex + direction];
                        let prevPoint = n
                            ? ring[(n + activeIndex - direction) % n]
                            : ring[activeIndex - direction];
                        if (nextPoint && prevPoint) {
                            projection = geotools.pointToLineProjection(prevPoint, [
                                ring[activeIndex],
                                nextPoint,
                            ]);
                            if (
...
sGis.module('Feature', ['utils', 'CRS', 'Bbox', 'EventHandler'], function(
    utils,
    CRS,
    Bbox,
    EventHandler,
...
sGis.module('feature.Image', ['utils', 'Feature', 'symbol.image.Image'], function(
    utils,
    Feature,
    ImageSymbol,
) {
...
sGis.module('feature.Label', ['utils', 'Feature', 'symbol.label.Label', 'Bbox', 'Point'], function(
    utils,
    Feature,
    LabelSymbol,
    Bbox,
...
sGis.module('feature.Maptip', ['Feature', 'Point', 'Bbox', 'symbol.maptip.Simple'], function(
    Feature,
    Point,
    Bbox,
    MaptipSymbol,
    ...
    class Maptip extends Feature {
        ...
        projectTo(crs) {
            ...
            return new Maptip(projected.position, {
                crs: crs,
...
sGis.module(
    'feature.MultiPoint',
    ['Feature', 'Point', 'Bbox', 'feature.Point', 'symbol.point.Point'],
    function(Feature, Point, Bbox, PointF, PointSymbol) {
        ...
        class MultiPoint extends Feature {
            ...
            projectTo(crs) {
                ...
                return new MultiPoint(projected, {
                    symbol: this.symbol,
                    crs: crs,
                });
            }
...
sGis.module(
    'feature.Point',
    ['utils', 'Feature', 'Crs', 'Point', 'Bbox', 'symbol.point.Point'],
    function(utils, Feature, Crs, Point, Bbox, PointSymbol) {
        ...
        class PointF extends Feature {
            ...
            projectTo(crs) {
                ...
                return new PointF(projected.position, {
                    crs: crs,
                    symbol: this.symbol,
                });
            }
...
sGis.module('feature.Poly', ['utils', 'Feature', 'Bbox', 'geotools'], function(
    utils,
    ...
    class Poly extends Feature {
        ...
        setPoint(ringN, pointN, point) {
            ...
            this._rings[ringN][pointN] =
                point.position && point.projectTo ? point.projectTo(this.crs).position : point;
            this._update();
        }
...
sGis.module('feature.Polygon', ['feature.Poly', 'symbol.polygon.Simple'], function(
    Poly,
    PolygonSymbol,
) {
    'use strict';
...
sGis.module('feature.Polyline', ['feature.Poly', 'symbol.polyline.Simple'], function(
    Poly,
    PolylineSymbol,
) {
    'use strict';
    ...
    class Polyline extends Poly {
        /**
			 ...
			 */
        clone() {
            return new Polyline(this.rings, {
                crs: this.crs,
                symbol: this.originalSymbol,
            });
        }
...
sGis.module('render.Image', ['Point'], Point => {
    'use strict';
...
sGis.module('render.Polygon', ['utils', 'geotools'], function(utils, geotools) {
    ...
    class Polygon {
        ...
        contains(position) {
            return geotools.contains(
                this.coordinates,
                position,
                this.strokeWidth / 2 + this.lineContainsTolerance,
            );
        }
    }
        ...
        contains(position) {
            for (var ring = 0, l = this.coordinates.length; ring < l; ring++) {
                for (var i = 1, m = this.coordinates[ring].length; i < m; i++) {
                    if (
                        geotools.pointToLineDistance(position, [
                            this.coordinates[ring][i - 1],
                            this.coordinates[ring][i],
                        ]) <
                        this.strokeWidth / 2 + this.lineContainsTolerance
                    )
                        return [ring, i - 1];
...
sGis.module('render.VectorImage', {}, () => {
    ...
    class VectorImage {
        ...
        contains(position) {
            ...
            return (
                position[0] >= x &&
                position[0] <= x + this._node.width &&
                position[1] >= y &&
                position[1] <= y + this._node.height
            );
        }
    }
...
sGis.module('render.VectorLabel', ['render.VectorImage'], VectorImage => {
    'use strict';
    ...
    class VectorLabel extends VectorImage {
        ...
        get hAlign() {
            return this.align.indexOf('right') >= 0
                ? 1
                : this.align.indexOf('center') >= 0 ? 0 : -1;
        }
        get vAlign() {
            return this.align.indexOf('bottom') >= 0
                ? 1
                : this.align.indexOf('middle') >= 0 ? 0 : -1;
        }
...
sGis.module('serializer.symbolSerializer', ['utils', 'utils.Color'], (utils, Color) => {
    'use strict';
    ...
    return {
        registerSymbol: (constructor, description, properties) => {
            symbolDescriptions[description] = {
            ...
            renderFunction(feature, resolution, crs) {
                ...
                for (var i = 0; i < baseRender.length; i++) {
                    if (baseRender[i] instanceof ArcRender) {
                        ...
                        this.baseSymbol instanceof PointImageSymbol ||
                        this.baseSymbol instanceof PointMaskedImageSymbol
                    ) {
                        halo = new ArcRender(
                            [
                                baseRender[i].position[0] -
                                    +this.baseSymbol.anchorPoint.x +
                                    this.baseSymbol.width / 2,
                                baseRender[i].position[1] -
                                    +this.baseSymbol.anchorPoint.x +
                                    this.baseSymbol.width / 2,
                            ],
                            {
...
sGis.module(
    'symbol.image.Image',
    ['Symbol', 'render.Image', 'serializer.symbolSerializer'],
                    ...
                    render.onAfterDisplayed = node => {
                        setTimeout(() => {
                            node.style.transition =
                                'opacity ' + this.transitionTime / 1000 + 's linear';
                            node.style.opacity = this.opacity;
                        }, 0);
                    };
                } else {
...
sGis.module('symbol.label.Label', ['utils', 'Symbol', 'render.HtmlElement'], function(
    utils,
    Symbol,
    HtmlElement,
) {
...
sGis.module(
    ...
    ['utils', 'Symbol', 'render.Polygon', 'render.HtmlElement'],
    function(utils, Symbol, PolygonRender, HtmlElement) {
        ...
        class MaptipSymbol extends Symbol {
            ...
            renderFunction(feature, resolution, crs) {
                ...
                let render = new HtmlElement(
                    `<div class="sGis-maptip-outerContainer"><div class="sGis-maptip-innerContainer">${feature.content}</div></div>`,
                    pxPosition,
                );
...
sGis.module(
    ...
    ['utils', 'math', 'Symbol', 'render.Polyline', 'serializer.symbolSerializer'],
    function(utils, math, Symbol, Polyline, symbolSerializer) {
        ...
        class PolylineSymbol extends Symbol {
            ...
            static _getRenderedCoordinates(feature, resolution, crs) {
                if (
                    !feature.coordinates ||
                    !utils.isArray(feature.coordinates) ||
                    !utils.isArray(feature.coordinates[0])
                )
                    return null;
                var projected = feature.crs.equals(crs)
                    ? feature.rings
                    : feature.projectTo(crs).rings;

                return math.simplifyCoordinates(
                    projected.map(ring => {
        ...
        symbolSerializer.registerSymbol(PolylineSymbol, 'polyline.Simple', [
            'strokeColor',
            'strokeWidth',
        ]);
...
sGis.module('utils.Color', ['utils'], utils => {
    'use strict';
    ...
    class Color {
        ...
        toString(format) {
            if (format === 'hex') {
                return (
                    '#' + decToHex(this.a) + decToHex(this.r) + decToHex(this.g) + decToHex(this.b)
                );
            } else if (format === 'rgb') {
                ...
                return (
                    ...
                    this.b +
                    ',' +
                    (this.a / 255).toFixed(2).replace(/\.*0+$/, '') +
                    ')'
                );
            }
        }
        ...
        get isValid() {
            return !!(
                utils.isNumber(this._channels.a) &&
                utils.isNumber(this._channels.r) &&
                utils.isNumber(this._channels.g) &&
                utils.isNumber(this._channels.b)
            );
        }
    ...
    var formats = {
        ...
        rgb: function(/** String */ string) {
            ...
            if (!percents || percents.length === 3) {
                var channels = string
                    .substring(string.search(/\(/) + 1, string.length - 1)
                    .split(',');
                for (var i = 0; i < 3; i++) {
                    if (channels[i]) {
                        ...
                        if (percent) {
                            ...
                            channels[i] =
                                channels[i] &&
                                channels[i].match(/[^ \-0-9]/) === null &&
                                channels[i].match(/[0-9]+/g).length === 1
                                    ? parseInt(channels[i])
                                    : NaN;
                            if (channels[i] < 0) {
                                channels[i] = 0;
        ...
        simplifyCoordinates: function(rings, tolerance) {
            ...
            for (var ring = 0, l = rings.length; ring < l; ring++) {
                ...
                for (var i = 1, len = rings[ring].length - 1; i < len; i++) {
                    if (
                        rings[ring][i].length === 0 ||
                        simplified[simplified.length - 1].length === 0 ||
                        Math.abs(rings[ring][i][0] - simplified[simplified.length - 1][0]) >
                            tolerance ||
                        Math.abs(rings[ring][i][1] - simplified[simplified.length - 1][1]) >
                            tolerance
                    ) {
...
sGis.module('utils.StateManager', ['utils'], (/** sGis.utils */ utils) => {
    'use strict';
    ...
    class StateManager {
        ...
        constructor(maxStates = 256) {
            if (!utils.isNumber(maxStates) || maxStates < 0)
                utils.error('Incorrect value for number of states: ' + maxStates);
...
sGis.module('utils', ['event'], function(ev) {
    ...
    var utils = {
        ...
        isImage: function(o) {
            return (
                (utils.browser.indexOf('Opera') !== 0 && o instanceof Image) ||
        ...
        validatePositiveNumber: function(n) {
            if (!utils.isNumber(n) || n <= 0)
                utils.error('Positive number is expected but got ' + n + ' instead');
        },
        ...
        validateBool: function(b) {
            if (b !== true && b !== false)
                utils.error('Boolean is expected but got ' + b + ' instead');
        },
            ...
            transition:
                document.body.style.transition !== undefined
                    ...
                    : document.body.style.webkitTransition !== undefined
                      ? { func: 'webkitTransition', rule: '-webkit-transition' }
                      : document.body.style.msTransition !== undefined
                        ? { func: 'msTransition', rule: '-ms-transition' }
                        : document.body.style.OTransition !== undefined
                          ? { func: 'OTransition', rule: '-o-transition' }
                          : null,
            transform:
                    ...
                    : document.body.style.webkitTransform !== undefined
                      ...
                      : document.body.style.OTransform !== undefined
                        ...
                        : document.body.style.msTransform !== undefined
                          ? { func: 'msTransform', rule: '-ms-transform' }
                          : null,
            transformOrigin:
                document.body.style.transformOrigin !== undefined
                    ? { func: 'transformOrigin', rule: 'transform-origin' }
    ...
    (Arc, Point, Polygon, Polyline, VectorImage, utils) => {
        ...
        class Canvas {
            ...
            reset(bbox, resolution, width, height) {
                ...
                this._ctx.translate(
                    Math.round(-bbox.xMin / resolution),
                    Math.round(bbox.yMax / resolution),
                );
            }
...
sGis.module('painter.domPainter.Container', ['utils'], utils => {
    let containerStyle =
        'width: 100%; height: 100%; transform-origin: left top; position: absolute;';
    ...
    (
        ...
        class DomRenderer {
            ...
            show(bbox, animate = true) {
                ...
                this.map[method](
                    projected.center,
                    this.map.getAdjustedResolution(Math.max(xResolution, yResolution)),
                );
            }
            ...
            _setNewContainer() {
                this._containers.push(
                    new Container(
                        this._layerWrapper,
                        this.bbox,
                        this._map.resolution,
                        this._removeEmptyContainers.bind(this),
                    ),
                );
            }
            ...
            _updateSize() {
                this._width = this._wrapper
                    ? this._wrapper.clientWidth || this._wrapper.offsetWidth
                    : 0;
                this._height = this._wrapper
                    ? this._wrapper.clientHeight || this._wrapper.offsetHeight
                    : 0;
            }
            ...
            _updateBbox() {
                ...
                if (
                    this._position[0] !== mapPosition[0] ||
                    this._position[1] !== mapPosition[1] ||
                    !utils.softEquals(this._map.resolution, this._resolution) ||
                    ...
                    this._bbox = new Bbox(
                        [mapPosition[0] - dx, mapPosition[1] - dy],
                        [mapPosition[0] + dx, mapPosition[1] + dy],
                        this._map.crs,
                    );
                    ...
                    if (
                        ...
                        this._containers[this._containers.length - 1].scale !== 1
                    )
                        this._needUpdate = true;
            ...
            getPointFromPxPosition(x, y) {
                ...
                let bbox = this.bbox;
                return new Point(
                    [bbox.xMin + x * resolution, bbox.yMax - y * resolution],
                    bbox.crs,
                );
            }
...
sGis.module('painter.domPainter.EventDispatcher', ['event', 'utils'], (ev, utils) => {
    'use strict';
    ...
    class EventDispatcher {
        ...
        _dispatchEvent(name, data) {
            ...
            if (data.position) {
                ...
                for (var i = layerRenderers.length - 1; i >= 0; i--) {
                    var details = {};
                    var targetObject = layerRenderers[i].getEventCatcher(
                        name,
                    ...
                    if (name === 'mousemove' && !targetObject) {
                        targetObject = layerRenderers[i].getEventCatcher(
                            'mouseover',
                            [data.position.x, data.position.y],
                            details,
                        );
        ...
        _onDocumentMousemove(event) {
            ...
            if (Math.abs(dxPx) > 2 || Math.abs(dyPx) > 2 || !this._clickCatcher) {
                ...
                if (this._clickCatcher) {
                    ...
                    var originalPoint = this._master.getPointFromPxPosition(
                        this._dragPosition.x,
                        this._dragPosition.y,
                    );
                    var originalPosition = {
                        x: originalPoint.x / resolution,
                        y: -originalPoint.y / resolution,
                    };
                    var sGisEvent = this._dispatchEvent('dragStart', {
                        map: map,
                ...
                this._draggingObject.fire('drag', {
                    map: map,
                    mouseOffset: mousePosition,
                    position: position,
                    point: point,
        ...
        _onwheel(event) {
            ...
            if (time - this._wheelTimer > MIN_WHEEL_DELAY) {
                ...
                map.zoom(
                    wheelDirection,
                    this._master.getPointFromPxPosition(mouseOffset.x, mouseOffset.y),
                );
            }
            event.preventDefault();
            return false;
        ...
        _getMouseEventDescription(event) {
            ...
            var position = {
                x: point.x / map.resolution,
                y: -point.y / map.resolution,
            };
            return {
                map: map,
        ...
        _ontouchmove(event) {
            ...
            if (touches.length === 1 && this._touchHandler.lastDrag) {
                ...
                let position = {
                    x: point.x / resolution,
                    y: 0 - point.y / resolution,
                };

                if (this._touchHandler.lastDrag.x === 0 && this._touchHandler.lastDrag.y === 0) {
                    let sGisEvent = this._dispatchEvent('dragStart', {
                        point: point,
                        position: position,
                        offset: {
                            xPx: dxPx,
                            yPx: dyPx,
                            x: this._touchHandler.lastDrag.x,
                            y: this._touchHandler.lastDrag.y,
                        },
                    });
                    this._draggingObject = sGisEvent.draggingObject || map;
                }

                this._touchHandler.lastDrag = {
                    x: dxPx * resolution,
                    y: 0 - dyPx * resolution,
                };
                this._draggingObject.fire('drag', {
        ...
        _ontouchend(event) {
            ...
            for (let i = 0; i < event.changedTouches.length; i++) {
                let index = this._touches.findIndex(
                    touch => touch.id === event.changedTouches[i].identifier,
                );
...
sGis.module(
    'painter.domPainter.LayerRenderer',
    ['Bbox', 'painter.domPainter.Canvas', 'painter.domPainter.SvgRender', 'utils'],
    (
        /** sGis.Bbox */ Bbox,
        ...
        class LayerRenderer {
            ...
            _rerender() {
                ...
                if (this._canvas.isEmpty) {
                    ...
                    if (this._canvasContainer) this._canvasContainer.removeNode(this._canvas.node);
                    this._master.currContainer.addNode(
                        this._canvas.node,
                        this._master.width,
                        this._master.height,
                        this._bbox,
            ...
            _drawNodeRender(render, feature) {
                ...
                var callback = (error, node) => {
                    ...
                    if (
                        !this._featureRenders.has(feature) ||
                        (!render.baseRender &&
                            this._featureRenders.get(feature).indexOf(render) < 0) ||
                        (render.baseRender &&
                            this._featureRenders.get(feature).indexOf(render.baseRender) < 0) ||
                        this._outdatedFeatureRenders.has(render) ||
                        this._rendersForRemoval.has(render)
                    )
                        return;
                    ...
                    if (node && container) {
                        if (container !== lastContainer) {
                            if (render.bbox) {
                                ...
                                lastContainer.addFixedSizeNode(
                                    node,
                                    [render.position[0] * k, render.position[1] * k],
                                    render.offset,
                                );
                            } else {
                                let svgRender = new SvgRender(
                                    render,
                                    container.resolution / lastContainer.resolution,
                                );
                                svgRender.getNode((err, newNode) => {
                                    if (!svgRender.position || !this._renderNodeMap.has(render))
                                        return;
...
sGis.module(
    ...
    ['render.Arc', 'render.Point', 'render.Polyline', 'render.Polygon', 'utils.Color', 'utils'],
        ...
        class SvgRender {
            ...
            _setNode() {
                if (this._baseRender instanceof Arc) {
                    if (
                        this._baseRender.startAngle == 0 ||
                        this._baseRender.endAngle == 2 * Math.PI
                    ) {
                        this._setArcNode();
                    } else {
            ...
            _setPolygonNode() {
                ...
                this._node = this._getPathNode({
                    ...
                    'stroke-dasharray':
                        this._baseRender.lineDash && this._baseRender.lineDash.length > 0
                            ? this._baseRender.lineDash.join(',')
                            : undefined,
                    'stroke-width': this._baseRender.strokeWidth,
                    fill:
                        this._baseRender.fillStyle === 'color'
                            ? this._baseRender.fillColor
                            : undefined,
                    fillImage:
                        this._baseRender.fillStyle === 'image'
                            ? this._baseRender.fillImage
                            : undefined,
                    width: path.width,
                    height: path.height,
                    x: path.x,
            ...
            _getSegment() {
                ...
                var largeFlag =
                    Math.abs(this._baseRender.endAngle - this._baseRender.startAngle) %
                        (Math.PI * 2) >
                    Math.PI
                        ? 1
                        : 0;
            ...
            _setArcNode() {
                ...
                var x =
                    this._baseRender.center[0] * this._adjK -
                    this._baseRender.radius -
                    this._baseRender.strokeWidth / 2;
                var y =
                    this._baseRender.center[1] * this._adjK -
                    this._baseRender.radius -
                    this._baseRender.strokeWidth / 2;
...
sGis.module(
    'symbol.point.Image',
    ['Symbol', 'render.HtmlElement', 'serializer.symbolSerializer'],
    (Symbol, HtmlElement, symbolSerializer) => {
        ...
        class ImageSymbol extends Symbol {
            ...
            renderFunction(/** sGis.feature.Point */ feature, resolution, crs) {
                ...
                var html =
                    ...
                    '>';
                return [
                    new HtmlElement(html, renderPosition, null, [
                        -this.anchorPoint.x,
                        -this.anchorPoint.y,
                    ]),
                ];
		 ...
		 */
        ImageSymbol.prototype.source =
            'data:image/png;base64,';

        symbolSerializer.registerSymbol(ImageSymbol, 'point.Image', [
            'width',
            'height',
            'anchorPoint',
            'source',
...
sGis.module(
    'symbol.point.MaskedImage',
    ['Symbol', 'render.HtmlElement', 'utils.Color', 'serializer.symbolSerializer'],
    (Symbol, HtmlElement, Color, symbolSerializer) => {
        ...
        //noinspection SpellCheckingInspection
        MaskedImage.prototype._imageSource =
            'data:image/png;base64,';
        //noinspection SpellCheckingInspection
        MaskedImage.prototype._maskSource =
            'data:image/png;base64,';
        MaskedImage.prototype._maskColor = '#9bdb00';

        symbolSerializer.registerSymbol(MaskedImage, 'point.MaskedImage', [
            'width',
            'height',
            'anchorPoint',
            'imageSource',
...
sGis.module(
    'symbol.point.Point',
    ['Symbol', 'render.Arc', 'serializer.symbolSerializer'],
    (Symbol, ArcRender, symbolSerializer) => {
        ...
        class PointSymbol extends Symbol {
            ...
            renderFunction(/** sGis.feature.Point */ feature, resolution, crs) {
                ...
                var position = feature.projectTo(crs).position;
                var pxPosition = [
                    position[0] / resolution + this.offset.x,
                    -position[1] / resolution + this.offset.y,
                ];

                var point = new ArcRender(pxPosition, {
        ...
        symbolSerializer.registerSymbol(PointSymbol, 'point.Point', [
            'size',
            'offset',
            'fillColor',
            'strokeColor',
...
sGis.module(
    'symbol.point.Square',
    ['Symbol', 'render.Polygon', 'serializer.symbolSerializer'],
    (Symbol, PolygonRender, symbolSerializer) => {
        ...
        symbolSerializer.registerSymbol(SquareSymbol, 'point.Square', [
            'size',
            'offset',
            'fillColor',
            'strokeColor',
...
sGis.module(
    ...
    function(Symbol, PolylineSymbol, PolygonRender, Color, symbolSerializer) {
                ...
                for (var i = 0, l = brush.length; i < l; i++) {
                    for (var j = 0, m = brush[i].length; j < m; j++) {
                        var srcA = brush[i][j] * foreground.a / ALPHA_NORMALIZER,
                            ...
                            r = Math.round(
                                Math.min(255, background.r * dstA + foreground.r * srcA),
                            ),
                            g = Math.round(
                                Math.min(255, background.g * dstA + foreground.g * srcA),
                            ),
                            b = Math.round(
                                Math.min(255, background.b * dstA + foreground.b * srcA),
                            );
        ...
        symbolSerializer.registerSymbol(PolygonSymbol, 'polygon.BrushFill', [
            'fillBrush',
            'fillBackground',
            'fillForeground',
            'strokeColor',
...
sGis.module(
    'symbol.polygon.ImageFill',
    ['Symbol', 'symbol.polyline.Simple', 'render.Polygon', 'serializer.symbolSerializer'],
    function(Symbol, PolylineSymbol, PolygonRender, symbolSerializer) {
        ...
        symbolSerializer.registerSymbol(PolygonSymbol, 'polygon.ImageFill', [
            'src',
            'strokeColor',
            'strokeWidth',
        ]);
...
sGis.module(
    ...
    function(Symbol, PolylineSymbol, PolygonRender, symbolSerializer) {
        ...
        symbolSerializer.registerSymbol(PolygonSymbol, 'polygon.Simple', [
            'fillColor',
            'strokeColor',
            'strokeWidth',
        ]);
...
sGis.module(
    ...
    ['utils', 'Control', 'Map', 'Layer', 'event', 'utils.proto'],
    function(utils, Control, Map, Layer, ev, proto) {
        ...
        var BaseLayerSwitch = function BaseLayerSwitch(painter, options) {
            ...
            if (!(map instanceof sGis.Map))
                sGis.utils.error('sGis.Map instance is expected but got ' + map + ' instead');
            this._map = map;
            ...
            if (options && options.layerDescriptions)
                this.layerDescriptions = options.layerDescriptions;
        };
        ...
        BaseLayerSwitch.prototype = {
            ...
            addLayer: function addLayer(layer, imageSrc) {
                if (!(layer instanceof sGis.Layer))
                    sGis.utils.error(
                        'sGis.Layer instance is expected but got ' + layer + ' instead',
                    );
                if (!layer.tileScheme)
                    sGis.utils.error(
                        'A layer without tile scheme cannot be interpreted as base layer',
                    );
                if (this.getLayerIndex(layer) !== -1)
                    sGis.utils.error('The layer is already in the list');

                this._layerDescriptions.push({
                    layer: layer,
                    imageSrc: imageSrc,
                });
            ...
            removeLayer: function removeLayer(layer) {
                if (this._activeLayer === layer) {
                    if (this._layerDescriptions.length === 1) {
                        ...
                        this.activeLayer = this._layerDescriptions[
                            layerIndex === 0 ? 1 : layerIndex - 1
                        ];
                    }
                }
            ...
            _addLayerToImageBox: function _addLayerToImageBox(layer) {
                ...
                if (index < this._inactiveLayerBox.children.length) {
                    this._inactiveLayerBox.insertBefore(
                        this._layerDescriptions[index].image,
                        this._inactiveLayerBox.children[index],
                    );
                } else {
            ...
            _createScroll: function _createScroll() {
                ...
                this._next.className =
                    'button everGis-navigation-button everGis-navigation-forward scrollButton';
                this._prev.className =
                    'button everGis-navigation-button everGis-navigation-back scrollButton';
            ...
            _updateImagePositions: function _updateImagePositions() {
                ...
                for (var i = 0, len = this._layerDescriptions.length; i < len; i++) {
                    ...
                    this._layerDescriptions[i].image.style.left =
                        i * (this._inactiveWidth + this._margin) + 'px';
                }
            },
            ...
            _removeLayerFromImageBox: function _removeLayerFromImageBox(layer) {
                this._inactiveLayerBox.removeChild(
                    this._layerDescriptions[this.getLayerIndex(layer)].image,
                );
            },
            ...
            _setActiveLayerImage: function _setActiveLayerImage() {
                ...
                if (this._activeLayerImageContainer.children.length > 0) {
                    this._activeLayerImageContainer.removeChild(
                        this._activeLayerImageContainer.children[0],
                    );
                }

                var index = this.getLayerIndex(this._activeLayer);
                if (!this._layerDescriptions[index].image) {
                    this._layerDescriptions[index].image = this._getLayerImageObject(
                        this._activeLayer,
                    );
                }
                ...
                if (images && images[0]) {
                    this._activeLayerImageContainer.style.backgroundImage =
                        'url(' + images[0].src + ')';
                }
            },
            ...
            _updateInactiveLayersDecoration: function _updateInactiveLayersDecoration() {
                ...
                for (var i = 0, len = this._layerDescriptions.length; i < len; i++) {
                    ...
                    if (index === -1 && isActive) {
                        ...
                        image.className =
                            image.className.substr(0, index - 1) +
                            image.className.substr(index + this.pickerActiveCss.length);
                    }
                }
            },
        };
        ...
        sGis.utils.proto.setProperties(BaseLayerSwitch.prototype, {
            ...
            isActive: {
                ...
                set: function set(bool) {
                    if (bool) {
                        if (this._painter.innerWrapper)
                            this._painter.innerWrapper.appendChild(this._container);
                        this._isActive = true;
                        this.fire('activate');
                    } else {
                        if (this._painter.innerWrapper && this._container.parentNode)
                            this._painter.innerWrapper.removeChild(this._container);
            ...
            activeCss: {
                ...
                set: function set(css) {
                    if (this._activeLayerImageContainer)
                        this._activeLayerImageContainer.className = css;
                    this._activeCss = css;
                },
            },
            pickerCss: {
        ...
        var defaultCss =
                '.sGis-control-baseLayerSwitch-container {position: absolute; right: 32px; bottom: 32px; width: 64px; height: 64px;} ' +
...
sGis.module(
    'controls.Area',
    ['Control', 'controls.Polygon', 'Map', 'feature.Label', 'geotools', 'utils.proto'],
    function(Control, PolygonControl, Map, Label, geotools, proto) {
        ...
        var Area = function Area(map, options) {
            ...
            this._polygonControl = new sGis.controls.Polygon(map, {
                activeLayer: options && options.activeLayer,
                style: {
                    strokeWidth: 2,
                    strokeColor: 'red',
            ...
            this._polygonControl.addListener('drawingBegin', function() {
                if (this.activeLayer.features.length > 1)
                    this.activeLayer.features = [
                        this.activeLayer.features[this.activeLayer.features.length - 1],
                    ];

                var feature = this._activeLayer.features[this._activeLayer.features.length - 1],
                    label = new sGis.feature.Label(feature.centroid, {
                        content: '',
                        crs: feature.crs,
                        style: {
                            css: 'sGis-distanceLabel',
                            offset: { x: -50, y: -10 },
                            width: 120,
                        },
                    });
...
sGis.module(
    'controls.Distance',
    ['utils', 'utils.proto', 'Map', 'controls.Polyline', 'geotools', 'Control'],
    function(utils, proto, Map, Polyline, geotools, Control) {
        ...
        var Distance = function Distance(map, options) {
            ...
            this._polylineControl = new sGis.controls.Polyline(map, {
                activeLayer: options && options.activeLayer,
                style: { strokeWidth: 2, strokeColor: 'red' },
            });
            sGis.utils.init(this, options);
            ...
            this._polylineControl.addListener('drawingBegin', function() {
                ...
                var feature = this.activeLayer.features[this.activeLayer.features.length - 1],
                    ...
                    label = new sGis.feature.Label(coord[1], {
                        symbol: new sGis.symbol.label.Label({
                            css: 'sGis-symbol-label-center-top sGis-distanceLabel',
                        }),
                        crs: map.crs,
                    });
        ...
        function addStyleSheet() {
            ...
            styleSheet.innerHTML =
                '.sGis-distanceLabel {font-family: "PT Sans",Tahoma; font-size: 15px; background-color: rgba(200, 200, 255, 0.8);border: 1px solid black;border-radius: 5px; color: black;}';
...
sGis.module('decorations.ScaleSlider', ['utils', 'utils.proto', 'EventHandler'], function(
    utils,
    proto,
    EventHandler,
) {
    ...
    var ScaleSlider = (function(_EventHandler) {
        ...
        function ScaleSlider(map, options) {
            ...
            var _this = _possibleConstructorReturn(
                this,
                (ScaleSlider.__proto__ || Object.getPrototypeOf(ScaleSlider)).call(this),
            );
        ...
        _createClass(ScaleSlider, [
            ...
            },
            {
                ...
                value: function _getSliderPosition() {
                    ...
                    var offset =
                        height *
                        Math.log2(curResolution / minResolution) /
                        Math.log2(maxResolution / minResolution);
            ...
            },
            {
                ...
                value: function _setSliderEvents() {
                    ...
                    this._map.addListener(
                        'layerAdd layerRemove bboxChangeEnd',
                        this._updateSliderPosition.bind(this),
                    );
                },
            },
            {
            ...
            },
            {
                ...
                value: function _moveSlider(delta) {
                    ...
                    this._map.resolution =
                        minResolution *
                        Math.pow(2, offset * Math.log2(maxResolution / minResolution) / height);
                },
            },
        ]);
    ...
    var defaultCss =
            '.sGis-decorations-scaleSlider-grid {' +
...
sGis.module('decorations.Scale', ['utils'], function(utils) {
    ...
    var Scale = function Scale(map, options) {
        sGis.utils.init(this, options);
        this._map = map;
        this.updateDisplay();
    };
    ...
    Scale.prototype = {
        _plusImageSrc:
            'data:image/png;base64,',
        _minusImageSrc:
            'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADAAAAAwCAYAAABXAvmHAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAAFpJREFUeNrs2LENwDAIAEETZTaGZjmsrODGQrkv6E+igejuNblnDQ8AAAAAAAAAAAAA4L+A9xtVNe4sy8ywQgAAAAAAALcLz10AAAAAAAAAAAAAAACAs7YAAwDJuQpbR1QAogAAAABJRU5ErkJggg==',
        _xAlign: 'left',
        _yAlign: 'top',
        _xOffset: 32,
        _yOffset: 32,
    ...
    var defaultCss =
            '.sGis-decorations-button {border: 1px solid gray; background-color: #F0F0F0; border-radius: 5px; font-size: 32px; text-align: center;cursor: pointer;} .sGis-decorations-button:hover {background-color: #E0E0E0;}',
        buttonStyle = document.createElement('style');
    buttonStyle.type = 'text/css';
    if (buttonStyle.styleSheet) {
        buttonStyle.styleSheet.cssText = defaultCss;
        ...
        _createClass(ZoomButtons, [
            {
                ...
                value: function _init(wrapper) {
                    var container = utils.createNode('div', this.wrapperClass, {}, [
                        utils.createNode('div', ZOOM_IN_CLASS, {
                            onclick: this._zoomIn.bind(this),
                        }),
                        utils.createNode('div', ZOOM_OUT_CLASS, {
            ...
            },
            {
                ...
                get: function get() {
                    ...
                    className +=
                        ' ' + (this._position.indexOf('right') >= 0 ? RIGHT_CLASS : LEFT_CLASS);
                    className +=
                        ' ' + (this._position.indexOf('bottom') >= 0 ? BOTTOM_CLASS : TOP_CLASS);
                    return className;
                },
            },
        ]);
...
sGis.module('sp.Api', ['utils', 'sp.serializers.JsonSerializer'], function(utils, JsonSerializer) {
    ...
    class Api {
                ...
                success: function(data) {
                    if (options.success) {
                        try {
                            ...
                            if (options.error)
                                options.error(
                                    'Failed to load user settings, unexpected response from server: ' +
                                        data,
                                );
                        }
                    }
        ...
        _publishService(type, params) {
            ...
            if (params.Name.length > 63)
                throw new Error('Name is not long. It cannot be longer then 63 symbols.');
        ...
        publishLayer({
            ...
            if (dataSourceName.length > 63)
                dataSourceName = dataSourceName.substr(dataSourceName.length - 62);
            ...
            return this.publishDataSource({
                ...
                if (!response || !response.Success)
                    throw new Error('Failed to publish service ' + name);
        ...
        deleteService(description) {
            return this._operation(
                ...
                JSON.stringify([description.serviceName]),
            );
        }
        ...
        pickByGeometry({ services, geometry, resolution }) {
            return this._operation('data/pickByGeometry', {
                services,
                geom: JSON.stringify(JsonSerializer.serializeGeometry(geometry)),
                res: resolution,
            }).then(response => {
        ...
        validateExpression({ targetServiceName, expression, resultType }) {
            return this._operation('functions/validateExpression', {
                targetServiceName,
                expression,
                resultType,
            });
        ...
        geocode(query, providers, crs = sGis.CRS.wgs84) {
            ...
            return this._operation('geocode', {
                sr,
                ...
                return response.map(item => {
                    if (crs === sGis.CRS.geo) {
                        ...
                        return {
                            ...
                            score: item.Score,
                            position: position,
                            point: new sGis.Point(position, crs),
                        };
                    } else {
                        return {
                            address: item.Address,
                            ...
                            point: new sGis.Point(item.Geometry, crs),
                        };
                    }
                });
            });
...
sGis.module('sp.ClusteringService', ['utils', 'FeatureLayer', 'feature.Point', 'Map'], function(
    utils,
    FeatureLayer,
    Point,
    Map,
    ...
    ClusteringService.prototype = {
        ...
        getClusters: function(options) {
            ...
            var bbox = options.bbox,
                ...
                sizeString =
                    Math.round(bbox.width / options.resolution) +
                    ',' +
                    Math.round(bbox.height / options.resolution);
            ...
            sGis.utils.ajax({
                url:
                    ...
                    options.storageId +
                    '/?bbox=' +
                    encodeURIComponent(bboxString) +
                    '&size=' +
                    encodeURIComponent(sizeString) +
                    this._serverConnector.sessionSuffix,
                cache: false,
                    ...
                    success: function(clusters) {
                        ...
                        clusters.forEach(function(cluster) {
                            var size =
                                self._minSize +
                                Math.round(
                                    (self._maxSize - self._minSize) *
                                        (cluster.Items.length / maxSize),
                                );
                            var point = new sGis.feature.Point([cluster.X, cluster.Y], {
                                crs: self._map.crs,
                                size: size,
                                color: 'red',
                            });
    ...
    Object.defineProperties(ClusteringService.prototype, {
        ...
        layer: {
            ...
            set: function(layer) {
                if (!(layer instanceof sGis.FeatureLayer))
                    sGis.utils.error(
                        'sGis.FeatureLayer instance is expected but got ' + layer + ' instead',
                    );
        ...
        storageId: {
            ...
            set: function(id) {
                if (!sGis.utils.isString(id))
                    sGis.utils.error('String is expected but got ' + id + ' instead');
        ...
        click: {
            ...
            set: function(handler) {
                if (!(handler instanceof Function))
                    sGis.utils.error('Function is expected but got ' + handler + ' instead');
                this._click = handler;
            },
        },
    });
...
sGis.module(
    'sp.ClusterLayer',
    ['Point', 'feature.Point', 'feature.Polygon', 'sp.ClusterSymbol', 'Layer', 'utils'],
    function(
        Point,
        PointF,
        Polygon,
        ...
        class ClusterLayer extends Layer {
            ...
            _getUrl(bbox, resolution) {
                return (
                    ...
                    this.clusterSize +
                    '&bbox=' +
                    bbox.coordinates.join('%2C') +
                    '&algorithm=' +
                    this.algorithm +
                    '&aggregationParameters=' +
                    encodeURIComponent(JSON.stringify(this.aggregationParameters)) +
        ...
        class ClusterSymbol extends PointSymbol {
            renderFunction(feature, resolution, crs) {
                ...
                if (this.pieAggregationIndex >= 0) {
                    let pieChart = this._applyChartClassifier(
                        feature,
                        renders[0].center,
                        renders[0].radius,
                    );
            ...
            _applySizeClassifier(circleRender, feature) {
                if (
                    feature.objectCount === undefined ||
                    !this.minSize ||
                    !this.maxSize ||
                    !this.sizeAggregationMaxValue
                )
                    return;
                ...
                let value =
                    this.sizeAggregationIndex <= 0
                        ? feature.objectCount
                        : feature.aggregations[this.sizeAggregationIndex].value;
            ...
            serialize() {
                return {
                    ...
                    singleObjectSymbol:
                        this.singleObjectSymbol &&
                        ((this.singleObjectSymbol.serialize &&
                            this.singleObjectSymbol.serialize()) ||
                            symbolSerializer.serialize(this.singleObjectSymbol)),
...
sGis.module(
    'sp.Connector',
    ['utils', 'EventHandler', 'sp.Api', 'sp.serializers.xmlSerializer'],
    function(utils, EventHandler, Api, xmlSerializer) {
        ...
        let ext = {
            ...
            removeNotificationListner: function(tag, string) {
                if (this._notificationListners[tag] && this._notificationListners[tag][string])
                    delete this._notificationListners[string];
            },
            ...
            initializeSession: function({ login, password, sessionId }) {
                this.initializationPromise = new Promise((resolve, reject) => {
                    ...
                    if (login && password) {
                        var spUrl =
                                this._url.substr(-4, 4) === 'IIS/'
                                    ? this._url.substr(0, this._url.length - 4)
                                    : this._url,
                            url =
                                this.apiLoginUrl.replace(/%sp%/, spUrl) +
                                '?userName=' +
                                login +
                                '&password=' +
                            ...
                            success: function(data, textStatus) {
                                if (data === '') {
                                    ...
                                    if (response.Success && response.Message) {
                                        ...
                                        reject(
                                            'Could not get session. Server responded with: ' + data,
                                        );
                                        sGis.utils.error(
                                            'Could not get session. Server responded with: ' + data,
                                        );
                    ...
                    xhr = sGis.utils.ajax({
                            ...
                            if (data && data.Notifications) {
                                for (var i in data.Notifications) {
                                    if (notificationProcessors[data.Notifications[i].tag]) {
                                        notificationProcessors[data.Notifications[i].tag](
                                            self,
                        ...
                        error: function(stringData, textStatus) {
                            ...
                            if (self._failedNotificationRequests > 5) {
                                ...
                                setTimeout(
                                    self.requestNotifications.bind(self),
                                    self._failedNotificationRequests * 1000,
                                );
                            }
                        },
                    });
            ...
            registerOperation: function(operationId, callback, progressCallback) {
                if (
                    this._latestOperationNotification &&
                    this._latestOperationNotification.operation.id === operationId
                ) {
                    callback(this._latestOperationNotification);
                    this._latestOperationNotification = null;
                } else {
                    this._operationList[operationId] = {
        ...
        let notificationProcessors = {
            'dynamic layer': function(connector, data, type) {
                if (
                    connector._notificationListners['dynamic layer'] &&
            ...
            DAS: function(connector, data, type) {
                ...
                if (connector._operationList[response.operation.id]) {
                    if (response.operation.status === 'Running') {
                        if (connector._operationList[response.operation.id].progressCallback)
                            connector._operationList[response.operation.id].progressCallback(
                                response,
                            );
                    } else {
            ...
            symbols: function(connector, data, type) {
                if (
                    connector._notificationListners['symbols'] &&
...
sGis.module(
    ...
    ['utils', 'LayerGroup', 'sp.controllers.ViewableController'],
    (utils, LayerGroup, ViewableController) => {
        ...
        class ControllerManager {
            ...
            createController(name) {
                ...
                let controller = new registry[name](this._connector, {
                    map: this._map,
                });
...
sGis.module(
    'sp.DataFilter',
    ['serializer.symbolSerializer', 'sp.Labeling', 'sp.ClusterSymbol', 'utils'],
        ...
        class DataFilter {
            ...
            serialize() {
                ...
                if (this.symbol instanceof ClusterSymbol) {
                    ...
                    serialized.SerializationData.clusterLabel =
                        this.labeling && this.labeling.serialize();
...
sGis.module('sp.DataOperation', ['sp.utils', 'EventHandler'], (utils, EventHandler) => {
    'use strict';
    ...
    class DataOperation {
        constructor(connector, controller, operationName, properties) {
            ...
            this._promise = new Promise((resolve, reject) => {
                controller.initializationPromise
                        ...
                        utils
                            ...
                            .then(data => {
                                ...
                                if (!response || response.status !== 'success')
                                    reject(
                                        `Unexpected response from server for ${this._controller
                                            .name}/${this._operationName} operation`,
                                    );
                                this.id = response.operationId;
                                connector.registerOperation(
                                    response.operationId,
                                    this._finalHandler.bind(this),
                    ...
                    .catch(() => {
                        reject(
                            `Controller ${controller.type} failed to initialize. Operation ${operationName} is cancelled.`,
                        );
                    });
            });
        ...
        cancel() {
            ...
            utils.ajaxp({
                url: `${this._connector.url}${this._controller.name}?cancel=${this.id}&_sb=${this
                    ._connector.sid}`,
            });
        }
    }
...
sGis.module(
    'sp.LayerManager',
    ['sp.ServiceGroup', 'sp.Project', 'sp.services.ServiceContainer', 'sp.DataFilter', 'utils'],
    function(ServiceGroup, Project, ServiceContainer, DataFilter, utils) {
        ...
        class LayerManager extends ServiceGroup {
            ...
            init(services = []) {
                Promise.all(
                    services.map(
                        name =>
                            new Promise(resolve =>
                                this.loadWithPromise(name).then(resolve).catch(resolve),
                            ),
                    ),
            ...
            loadService(name, index = -1, parent = null) {
                if (this.getServiceContainer(name, true))
                    throw new Error(`Service ${name} is already in the list`);
        ...
        Project.registerCustomDataItem(
            'services',
            ({ layerManager }) => {
            ...
            (descriptions, { layerManager }) => {
                ...
                descriptions.forEach(serviceDesc => {
                    let container = layerManager.getServiceContainer(
        ...
        function restoreService(layerManager, serviceDesc, parent) {
            if (serviceDesc.isFolder) {
                let service = new ServiceGroup(serviceDesc.serviceName, {
                    alias: serviceDesc.alias,
                });
                let container = new ServiceContainer(
                    layerManager._connector,
                    serviceDesc.serviceName,
                    { service },
                );
            ...
            layerManager
                .loadWithPromise(serviceDesc.serviceName, parent)
                .then(service => {
        ...
        function restoreServiceParameters(container, desc, layerManager) {
            ...
            if (desc.isDisplayed !== undefined && container.service)
                container.service.isDisplayed = desc.isDisplayed;
        ...
        function saveContainer(container) {
            return {
                ...
                filter:
                    container.service &&
                    ((container.service.tempFilterApplied &&
                        container.service.dataFilter &&
                        container.service.dataFilter.serialize()) ||
                        (container.service.view &&
                            container.service.view.tempFilterApplied &&
                            container.service.view.dataFilter &&
                            container.service.view.dataFilter.serialize())),
                customFilter:
                    container.service &&
...
sGis.module('sp.Printer', ['sp.utils'], function(utils) {
    ...
    Printer.prototype = {
        getTemplates: function() {
            return this._serverConnector.initializationPromise
                .then(() =>
                    utils.ajaxp({
                        url:
                            this._serverConnector.url +
                            'export/templates/' +
                            (this._serverConnector.sessionId
                                ? '?_sb=' + this._serverConnector.sessionId
                                : ''),
                        cache: false,
                    }),
                )
        ...
        getPreview: function(properties) {
            return this.__store(properties).then(() => {
                return (
                    this._serverConnector.url +
                    'export/preview/?noHeader=true&f=binary' +
                    this._serverConnector.sessionSuffix +
                    '&ts=' +
                    Date.now()
                );
        ...
        getImage: function(properties) {
            return this.__store(properties).then(() => {
                let link =
                    this._serverConnector.url +
                    'export/print/?noHeader=true&f=' +
                    (properties.useApi ? 'json' : 'binary') +
                    this._serverConnector.sessionSuffix +
                    '&ts=' +
                    Date.now() +
                    (properties.useApi ? '&asLink=true' : '');
        ...
        __store: function(properties) {
            var description = {
                ...
                PaperSize: {
                    Width:
                        (properties.paperSize && properties.paperSize.width) ||
                        defaults.paperSize.width,
                    Height:
                        (properties.paperSize && properties.paperSize.height) ||
                        defaults.paperSize.height,
                },
                Margin: {
                    ...
                    Bottom:
                        (properties.margin && properties.margin.bottom) || defaults.margin.bottom,
                },
            ...
            for (var i = 0, len = properties.template.BindingGroups.length; i < len; i++) {
                description.Parameters = description.Parameters.concat(
                    properties.template.BindingGroups[i].Parameters,
                );
            }
...
sGis.module('sp.Project', ['utils'], utils => {
    'use strict';
    ...
    class Project {
        ...
        load(name) {
            ...
            return this._api
                .operation('projects/load', { name: name })
                .then(response => {
                    this._isLoading = false;
                    this._name = name;
        ...
        class ServiceGroup extends EventHandler {
            ...
            getServiceContainer(serviceName, recurse = true) {
                ...
                for (let i = 0; i < this._children.length; i++) {
                    if (
                        this._children[i].name === serviceName ||
                        this._children[i].localName === serviceName
                    )
                        return this._children[i];
                    if (
                        recurse &&
                        this._children[i].service &&
                        this._children[i].service.children
                    ) {
                        let found = this._children[i].service.getServiceContainer(
                            serviceName,
                            true,
                        );
            ...
            getServices(recurse) {
                ...
                this._children.forEach(c => {
                    ...
                    if (recurse && c.service.getServices)
                        children = children.concat(c.service.getServices(true));
                });
                ...
                this._children.forEach(child => {
                    if (
                        ...
                        (recurse &&
                            ...
                            child.service.contains(container))
                    ) {
                        isContain = true;
                    }
                });
            ...
            getParent(container) {
                ...
                let groups = this.children.filter(
                    x => x.service && x.service instanceof ServiceGroup,
                );
    ...
    function(
        ...
        class SpatialProcessor {
            ...
            constructor(properties) {
                ...
                if (sessionId) {
                    this._connector = new Connector(url, authServiceUrl, {
                        sessionId,
                    });
                } else {
                    this._connector = new Connector(url, authServiceUrl, {
            ...
            _init({
                ...
                if (baseService) {
                    ...
                    this._baseServiceContainer.once(
                        'stateUpdate',
                        this._onBaseServiceInit.bind(this, {
                            position,
                            resolution,
                            mapWrapper,
                            centerPoint,
            ...
            _onBaseServiceInit(params) {
                if (!this._baseServiceContainer.service) {
                    console.error(
                        'Base service initialization failed. Error: ' +
                            this._baseServiceContainer.error,
                    );
                } else if (!(this._baseServiceContainer.service instanceof TileService)) {
                    console.error(
                        'Base service must be a tile service, but loaded service does not support tile rendering.',
                    );
                } else {
                ...
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        if (properties.success)
                            properties.success(xhr.responseText, xhr.statusText);
                    } else {
...
sGis.module('sp.controllers.Controller', ['sp.utils', 'sp.controllers.DataAccessBase'], function(
    utils,
    DataAccessBase,
) {
    'use strict';
    ...
    class Controller extends DataAccessBase {
        ...
        _createController() {
            return utils
                .ajaxp({
                    url: `${this._connector.url}${this.controllerServiceName}/?_sb=${this._connector
                        .sessionId}`,
                    type: 'POST',
...
sGis.module(
    ...
    ['EventHandler', 'sp.DataOperation', 'sp.serializers.xmlSerializer'],
    function(EventHandler, DataOperation, xmlSerializer) {
        ...
        class DataAccessBase extends EventHandler {
            ...
            init(initializationPromise) {
                this.initializationPromise = initializationPromise.then(
        ...
        class DataAccessService extends DataAccessBase {
            ...
            queryByGeometry(properties) {
                ...
                return this.operation(
                    'queryByGeometry',
                    { serviceName, geometry: serialized, resolution },
                    true,
                );
            }
            ...
            updateFeatures(properties) {
                ...
                let serialized = xmlSerializer.serializeGeometryEdit(
                    { updated: features },
                    false,
                    true,
                );
            ...
            createFeature(properties) {
                ...
                let serialized = JsonSerializer.serializeGeometry(geometry);
                return this.operation('createVisualObject', {
                    geometry: serialized,
                    serviceName,
                    attributes,
                });
            ...
            autoComplete(properties) {
                ...
                return this.operation('autoComplete', {
                    serviceName,
                    line: [serialized],
                    ids,
                });
            ...
            projectGeometry(properties) {
                ...
                return this.operation('gcProject', {
                    sourceGeom: geometry,
                    sourceSr,
                    destSr: destinationSr,
                });
            }

            getScalarValue(properties) {
                let { serviceName, query } = properties;
                return this.operation('selectScalarValue', {
                    serviceName,
                    query,
                });
            }
            ...
            batchEdit(properties) {
                ...
                return this.operation('batchFuncEdit', {
                    serviceName,
                    attribute,
                    expression,
                    condition,
            ...
            calculateBuffers(properties) {
                let {
                    distances,
                    unionResults = false,
                    subtractObject = false,
                    processDelay = null,
                    subtractInnerBuffer = false,
                    sourceServiceName,
                    targetServiceName,
                } = properties;
                return this.operation('buffer', {
                    distances,
                    unionResults,
                    subtractInnerBuffer,
                    subtractObject,
            ...
            buildIsochroneByStorage(properties) {
                ...
                return this.operation('isochroneByStorage', {
                    duration,
                    solver,
                    sourceServiceName,
                    targetServiceName,
    ...
    DitIntegration.prototype = new sGis.sp.Controller({
        ...
        disintegrate: function(properties) {
            ...
            this.__operation(function() {
                var param =
                    'layerId=' +
                    encodeURIComponent(properties.layerId) +
                    '&moduleId=' +
                    encodeURIComponent(properties.moduleId) +
                    '&shitId=' +
                    encodeURIComponent(properties.queryId);
                return {
                    operation: 'disintegrate',
...
sGis.module(
    'sp.controllers.ImportData',
    ['sp.controllers.Controller', 'sp.ControllerManager'],
...
sGis.module(
    'sp.controllers.ObjectSelector',
    ['sp.controllers.ViewableController', 'sp.ControllerManager', 'sp.serializers.JsonSerializer'],
        ...
        class ObjectSelector extends ViewableController {
            ...
            select(properties) {
                ...
                return this.operation('select', {
                    geom: serialized,
                    res: this.map && this.map.resolution,
                    mode,
                    services,
                    sr: this.map && this.map.crs.toString(),
                });
            }
            ...
            pickById(properties) {
                ...
                return this.operation(
                    'pickById',
                    {
                        ids: [{ ServiceName: serviceName, ObjectIds: ids }],
                        mode,
                    },
                    true,
                );
            ...
            selectByStorage(properties) {
                ...
                return this.operation('selectByStorage', {
                    geometryService,
                    res: this.map && this.map.resolution,
                    services,
                    mode,
...
sGis.module('sp.controller.Routing', ['sp.Controller', 'sp.ControllerManager'], function(
    Controller,
    ControllerManager,
) {
    'use strict';
        ...
        buildRoute: function(properties) {
            this.__operation(function() {
                var startPoint =
                    ...
                    encodeURIComponent(
                        JSON.stringify({
                            x: properties.startPoint.x,
                            y: properties.startPoint.y,
                            spatialReference: this._map.crs.getWkidString(),
                        }),
                    );
                var endPoint =
                    ...
                    encodeURIComponent(
                        JSON.stringify({
                            x: properties.endPoint.x,
                            y: properties.endPoint.y,
                            spatialReference: this._map.crs.getWkidString(),
                        }),
                    );
                var sr =
                    'spatialReference=' +
                    encodeURIComponent(JSON.stringify(this._map.crs.getWkidString()));
                        ...
                        : function(response) {
                              properties.success(
                                  self._createFeatures(
                                      response,
                                      properties.crs ||
                                          (properties.startPoint && properties.startPoint.crs) ||
                                          (self._map && self._map.crs),
                                  ),
                              );
                          },
                    error: properties.error,
...
sGis.module(
    ...
    ['sp.controllers.ViewableController', 'sp.ControllerManager', 'sp.services.ServiceContainer'],
    function(ViewableController, ControllerManager, ServiceContainer) {
        ...
        class TempView extends ViewableController {
            ...
            resetView(properties) {
                ...
                return this.operation('resetView', { sourceServiceName }).then(
...
sGis.module(
    ...
    ['sp.controllers.Controller', 'sp.services.ServiceContainer'],
    (Controller, ServiceContainer) => {
        ...
        class ViewableController extends Controller {
            ...
            _checkInitialization() {
                if (!this.initData.DataViewServiceName)
                    throw new Error(
                        `Controller ${this
                            .type} initialization failed: server did not return view name.`,
                    );
            }
                    ...
                    this.initializationPromise.then(() => {
                        ...
                        container.once('stateUpdate', () => {
                            if (container.service && container.service.layer) {
                                ...
                                reject(
                                    `Controller ${this.type} update failed: failed to update view.`,
                                );
                            }
                        });
                    });
...
sGis.module(
    ...
    ['Layer', 'sp.ClusterLayer', 'DynamicLayer', 'sp.ClusterSymbol'],
        ...
        class DataViewLayer extends Layer {
            ...
            _fillResolutionGroups(filter) {
                ...
                if (filter.symbol && filter.symbol instanceof ClusterSymbol) {
                    let layer = new ClusterLayer(
                        this._service.url,
                        this._service.connector.sessionId,
                        filter.symbol,
                    );
                    layer.aggregationParameters = [
                        {
                            filters: filter.condition,
                            aggregations: filter.aggregations && filter.aggregations.join(','),
                        },
                    ];
                    ...
                    this._resolutionGroups.push({
                        minResolution: filter.minResolution,
                        maxResolution: filter.maxResolution,
                        layer: layer,
                    });
                } else {
                    this._resolutionGroups.push({
            ...
            getFeatures(bbox, resolution) {
                ...
                if (this._resolutionGroups.length === 0)
                    return this._dynamicLayer.getFeatures(bbox, resolution);

                let dynamicLayerUsed = false;
                let features = [];
                this._resolutionGroups.forEach(group => {
                    if (
                        (group.minResolution > 0 && group.minResolution > resolution) ||
                        (group.maxResolution > 0 && group.maxResolution < resolution)
                    )
                        return;
            ...
            getImageUrl(bbox, resolution) {
                ...
                return (
                    ...
                    '&' +
                    'f=image' +
                    this._service.connector.sessionSuffix
                );
            }
    ...
    (utils, Crs, Point, Polyline, Polygon, PointSymbol, PolylineSymbol, PolygonSymbol) => {
        ...
        return {
            serializeGeometry(geometry) {
                ...
                if (geometry instanceof sGis.feature.Polygon) {
                    return {
                        ...
                        spatialReference: { wkid: crs.wkid },
                    };
                } else if (
                    geometry instanceof sGis.feature.Point ||
                    geometry instanceof sGis.Point
                ) {
                    return {
                        x: geometry.x,
                        y: geometry.y,
                        spatialReference: crs && crs.toString(),
                    };
            ...
            deserializeFeature(obj, crs) {
                ...
                return new Constructor(obj.geometry, {
                    crs: crs,
...
sGis.module(
    'sp.serializers.xmlSerializer',
    ['feature.Point', 'feature.Polyline', 'feature.Polygon', 'utils.Color', 'CRS'],
    function(Point, Polyline, Polygon, Color, CRS) {
        ...
        function createFeatures(response) {
            ...
            if (response.objects) {
                for (var i in response.objects) {
                    ...
                    let visualDefinition =
                        object.visualDefinition ||
                        (object.geometry && DEFAULT_VD[object.geometry.data.type]);
                    if (object.geometry && visualDefinition) {
                        var geometry = object.geometry.data,
                            points = geometry.coordinates,
                            attributes = object.attributes,
                            color = visualDefinition.stroke
                                ? parseColor(visualDefinition.stroke)
                                : undefined,
                        ...
                        var idAttribute =
                            response.attributesDefinitions[object.attributesDefinition]._identity;
                        var id = parseInt(object.attributes[idAttribute].value);

                        if (geometry.type === 'polygon') {
                            var feature = new sGis.feature.Polygon(points, {
                                id: id,
                                attributes: attributes,
                                crs: crs,
                                color: color,
                                width: visualDefinition.strokeThickness,
                            });
                            ...
                            let symbol = new sGis.symbol.polyline.Simple({
                                strokeColor: color,
                                strokeWidth: parseFloat(visualDefinition.strokeThickness),
                            });
                            feature = new sGis.feature.Polyline(points, {
                                id: id,
                                attributes: attributes,
                                crs: crs,
                                symbol: symbol,
                            ...
                            var featureClass =
                                geometry.type === 'point'
                                    ? sGis.feature.Point
                                    : sGis.feature.MultiPoint;
                            if (geometry.type === 'multipoint') points = points[0];
                            feature = new featureClass(points, {
                                id: id,
                                attributes: attributes,
                                crs: crs,
                                symbol: symbol,
                    ...
                    if (feature && response.attributesDefinitions && object.attributesDefinition) {
                        feature.displayField =
                            response.attributesDefinitions[object.attributesDefinition]._display;
        ...
        var serializer = {
            ...
            AttributesDefinition: function(node, parsed) {
                ...
                for (var i in node.childNodes) {
                    ...
                    if (
                        attributeInfo.childNodes.length > 0 &&
                        attributeInfo.childNodes[0].nodeName === 'Domain'
                    ) {
                        attributesDefinition[fields.name].domain = getDomainDescription(
                            attributeInfo,
                        );
                    }
                }
            ...
            ByteArray: function(node, parsed) {
                ...
                var attributes = getNodeAttributes(node);
                parsed.image[attributes.Key] = {
                    dataUrl: 'data:image/png;base64,' + node.childNodes[0].nodeValue,
                };
            },
            ...
            SimplePolygonSymbol: function(node, parsed) {
                ...
                parsed.symbol[attributes.Key] = {
                    ...
                    fill: parsed.brush[attributes.Fill]
                        ? parsed.brush[attributes.Fill].color || parsed.brush[attributes.Fill]
                        : undefined,
                    stroke: parsed.brush[attributes.Stroke]
                        ? parsed.brush[attributes.Stroke].color || 'FF000000'
                        : undefined,
                };
            },
            ...
            SimplePointSymbol: function(node, parsed) {
                ...
                parsed.symbol[attributes.Key] = {
                    ...
                    stroke: parsed.brush[attributes.Stroke]
                        ? parsed.brush[attributes.Stroke].color
                        : 'transparent',
            ...
            ImagePointSymbol: function(node, parsed) {
                ...
                parsed.symbol[attributes.Key] = {
                    ...
                    anchorPoint: {
                        x: attributes.AnchorPointX,
                        y: attributes.AnchorPointY,
                    },
            ...
            Geometric: function(node, parsed) {
                ...
                parsed.objects[nodeAttributes.Id] = {
                    ...
                    visualDefinition:
                        parsed.symbol && parsed.symbol[nodeAttributes.VisualDefinition]
                            ? parsed.symbol[nodeAttributes.VisualDefinition]
                            : null,
            ...
            Attribute: function(node, parsed, parentObject) {
                ...
                var attributeDefinition =
                    parsed.attributesDefinitions[parentObject.attributesDefinition][
                        nodeAttributes.Name
                    ];
            ...
            Geometry: function(node, parsed, parentObject) {
                if (parsed.geometryType === 'json') {
                    var attributes = getNodeAttributes(node),
                        ...
                        coordinates =
                            jsonData.type === 'point' ? [jsonData.x, jsonData.y] : jsonData.v;
                    parentObject.geometry = {
        ...
        function getNodeAttributes(node) {
            var keys = Object.keys(node.attributes),
                attributes = {};
            for (var i in keys) {
                if (typeof node.attributes[keys[i]] === 'object')
                    attributes[node.attributes[keys[i]].name] = node.attributes[keys[i]].nodeValue;
            }
            return attributes;
        }
     ...
     */
        ...
        xmlSerializer.serializeGeometryEdit = function(
            editDescription,
            attributesOnly,
            ignoreSymbol,
        ) {
            ...
            for (var i in editDescription) {
                if (sGis.utils.isArray(editDescription[i]) && i !== 'deleted')
                    featureList = featureList.concat(editDescription[i]);
            }
        ...
        sGis.sp.serializeSymbols = function(symbols) {
            ...
            for (var i = 0, len = symbols.length; i < len; i++) {
                features.push(
                    new featureClasses[symbols[i].type]([], {
                        symbol: symbols[i],
                    }),
                );
            }
        ...
        sGis.sp.serializeAttributes = function(attributes) {
            ...
            for (var i in attributes) {
                if (attributes.hasOwnProperty(i)) {
                    var attributesIndex = getAttributesDefinitionIndex(
                        attributes[i],
                        data.resources,
                    );
                    data.visualObjects[i] = {
                        attributesIndex: attributesIndex,
                        feature: { id: i, attributes: attributes[i] },
                    };
                }
            }
        ...
        function getXML(data, editDescription, attributesOnly, ignoreSymbol) {
            ...
            if (editDescription)
                dataNode.appendChild(getEditCommandsNode(editDescription, xml, attributesOnly));
        ...
        function getEditCommandsNode(editDescription, xml, attributesOnly) {
            ...
            if (sGis.utils.isArray(editDescription.updated)) {
                for (var i in editDescription.updated) {
                    node.appendChild(
                        getUpdateObjectNode(editDescription.updated[i], xml, attributesOnly),
                    );
                }
            }
        ...
        function getResourcesNode(data, xml, attributesOnly, ignoreSymbol) {
            ...
            for (var i in data.resources.attributesDefinitions) {
                node.appendChild(
                    getAttributesDefinitionNode(data.resources.attributesDefinitions[i], i, xml),
                );
            }
        ...
        function getAttributesDefinitionNode(attributeDefinition, key, xml) {
            ...
            for (var i in attributeDefinition) {
                if (
                    attributeDefinition[i].type ===
                    'Strategis.Server.SpatialProcessor.Core.ObjectId'
                ) {
                    var identity = i;
                }
            }
        ...
        function getVisualObjectsNode(data, xml, attributesOnly, ignoreSymbol) {
            ...
            for (var i in data.visualObjects) {
                if (data.visualObjects.hasOwnProperty(i)) {
                    node.appendChild(
                        getGeometricNode(data.visualObjects[i], xml, attributesOnly, ignoreSymbol),
                    );
                }
            }
        ...
        function getGeometricNode(visualObject, xml, attributesOnly, ignoreSymbol) {
            ...
            if (!attributesOnly && !ignoreSymbol) {
                ...
                nodeAttributes.VisualDefinitionId = visualObject.feature.visualDefinitionId
                    ? visualObject.feature.visualDefinitionId
                    : visualObject.feature.visualDefinitionId === undefined
                      ? undefined
                      : '00000000-0000-0000-0000-000000000000';
                nodeAttributes.GeneratorId = visualObject.feature.generatorId
                    ? visualObject.feature.generatorId
                    : visualObject.feature.generatorId === undefined
                      ? undefined
                      : '00000000-0000-0000-0000-000000000000';
            }
        ...
        function setNodeAttributes(node, attributes) {
            for (var i in attributes) {
                if (attributes[i] !== '' && attributes[i] !== undefined)
        ...
        function getSymbolIndex(feature, resources) {
            ...
            if (feature.type === 'point') {
                ...
                newSymbol = {
                    ...
                    Opacity: 1,
                    Fill: getBrushIndex(
                        symbol.fillColor ? symbol.fillColor : feature.strokeColor,
                        resources,
                    ),
                    Stroke: getBrushIndex(symbol.strokeColor, resources),
                    type: symbolTypes[feature.type],
        ...
        function getHatchBrushIndex(style, resources) {
            ...
            for (var i in resources.brushes) {
                if (
                    resources.brushes[i] instanceof Object &&
                    resources.brushes[i].brushString === brushString &&
                    resources.brushes[i].background === style.fillBackground &&
                    resources.brushes[i].foreground === style.fillForeground
                ) {
                    return i;
                }
            }
        ...
        function getAttributesDefinitionIndex(attributes, resources) {
            ...
            for (var i in attributesDefinitions) {
                ...
                for (var j in attributes) {
                    ...
                    if (
                        attributes[j].title !== attributesDefinitions[i][j].title ||
                        attributes[j].type !== attributesDefinitions[i][j].type
                    )
                        same = false;
                }
...
sGis.module(
    'sp.services.DataSourceService',
    ['EventHandler', 'sp.controllers.TempView', 'sp.services.ServiceContainer'],
    (EventHandler, TempView, ServiceContainer) => {
        ...
        class DataSourceService extends EventHandler {
            ...
            _initialize() {
                ...
                this._initializationPromise = this._tempViewController
                    .resetView({ sourceServiceName: this._name })
                    .then(() => {
            ...
            _setForwardListeners() {
                this._tempViewController.service.on(
                    'visibilityChange legendUpdate layerChange',
                    this.forwardEvent.bind(this),
                );
            }
    ...
    (utils, MapService, ClusterLayer, ServiceContainer, DataFilter, DataViewLayer) => {
        ...
        class DataViewService extends MapService {
            constructor(name, connector, serviceInfo) {
                ...
                if (serviceInfo.dataFilter)
                    this._dataFilter = this._originalFilter = DataFilter.deserialize(
                        serviceInfo.dataFilter,
                    );
                this._setLayer();
            ...
            get isFilterable() {
                return (
                    this.serviceInfo.capabilities &&
                    this.serviceInfo.capabilities.indexOf('setTempDataFilter') !== -1
                );
            }
            ...
            set showAsClusters(bool) {
                ...
                this.fire('layerChange', {
                    prevLayer: bool ? this.dynamicLayer : this.clusterLayer,
                });
            }
        ...
        ServiceContainer.register(
            serviceInfo =>
                (serviceInfo.serviceType === 'DataView' &&
                    serviceInfo.capabilities.indexOf('tile') === -1) ||
                serviceInfo.serviceType === 'DataSourceService',
            DataViewService,
        );
    ...
    (utils, CRS, EventHandler, Bbox) => {
        ...
        class MapService extends EventHandler {
            ...
            _subscribeForNotifications() {
                utils
                    .ajaxp({
                        url: this.url + 'subscribe?_sb=' + this._connector.sessionId,
                    })
                    .then(() => {
                        this._connector.addNotificationListner(
                            'dynamic layer',
                            this._name,
            ...
            get hasLegend() {
                return (
                    this.serviceInfo &&
                    this.serviceInfo.capabilities &&
                    this.serviceInfo.capabilities.indexOf('legend') >= 0
                );
            }

            updateLegend() {
                if (this.hasLegend)
                    return this._requestLegend().then(legend => {
                        try {
            ...
            _requestLegend() {
                return utils.ajaxp({
                    url:
                        this.url +
                        'legend' +
                        (this._connector.sessionId ? '?_sb=' + this._connector.sessionId : ''),
                });
            }
            ...
            get fullExtent() {
                ...
                if (
                    !this.serviceInfo.fullExtent ||
                    this.serviceInfo.fullExtent.xmin === this.serviceInfo.fullExtent.xmax
                )
                    return null;
                return new Bbox(
                    [this.serviceInfo.fullExtent.xmin, this.serviceInfo.fullExtent.ymin],
                    [this.serviceInfo.fullExtent.xmax, this.serviceInfo.fullExtent.ymax],
                    this.crs,
                );
            }
            ...
            get initialExtent() {
                if (
                    !this.serviceInfo.initialExtent ||
                    this.serviceInfo.initialExtent.xmin === this.serviceInfo.initialExtent.xmax
                )
                    return null;
                return new Bbox(
                    [this.serviceInfo.initialExtent.xmin, this.serviceInfo.initialExtent.ymin],
                    [this.serviceInfo.initialExtent.xmax, this.serviceInfo.initialExtent.ymax],
                    this.crs,
                );
            }
                    ...
                    return utils
                        .ajaxp({
                            url:
                                this.url +
                                'extent' +
                                (this._connector.sessionId
                                    ? '?_sb=' + this._connector.sessionId
                                    : ''),
                        })
                        .then(response => {
                            try {
...
sGis.module(
    'sp.services.ServiceContainer',
    ['FeatureLayer', 'EventHandler', 'utils', 'sp.utils'],
    (FeatureLayer, EventHandler, utils) => {
        ...
        class ServiceContainer extends EventHandler {
            ...
            _loadServiceInfo() {
                ...
                return this._connector.initializationPromise
                    .then(utils.ajaxp.bind(utils, { url }))
                    .then(([response]) => {
            ...
            _createService(serviceInfo) {
                for (let i = 0; i < serviceTypeRegistry.length; i++) {
                    if (serviceTypeRegistry[i].condition(serviceInfo)) {
                        this._service = new serviceTypeRegistry[i].constructor(
                            this._name,
                            this._connector,
                            serviceInfo,
                        );
...
sGis.module(
    'sp.services.ServiceGroup',
    ['sp.ServiceGroup', 'sp.services.ServiceContainer'],
    (ServiceGroup, ServiceContainer) => {
        'use strict';

        class ServiceGroupService extends ServiceGroup {
            constructor(name, connector, serviceInfo) {
                let children = serviceInfo.childrenInfo.map(
                    info =>
                        new ServiceContainer(connector, info.name, {
                            serviceInfo: info,
                            isDisplayed: serviceInfo.contents.find(({ name }) => name === info.name)
                                .isVisible,
                        }),
                );
        ...
        ServiceContainer.register(
            serviceInfo => serviceInfo.serviceType === 'LayerGroup',
            ServiceGroupService,
        );
...
sGis.module(
    'sp.services.StaticSourceService',
    ['sp.utils', 'sp.services.ServiceContainer', 'EventHandler'],
    (utils, ServiceContainer, EventHandler) => {
        ...
        class StaticSourceService extends EventHandler {
            ...
            upload(fileName, file) {
                ...
                return utils
                    .ajaxp({
                        url: `${this._url}upload?fileName=${fileName}${this._connector
                            .sessionSuffix}`,
                        type: 'POST',
                        data: data,
                    ...
                    .then(response => {
                        ...
                        if (respObject.success) {
                            return `${self._url}download/${fileName}${self._connector
                                .sessionSuffix}`;
            ...
            delete(fileName) {
                ...
                return utils
                    .ajaxp({
                        url: `${this._url}delete?fileName=${fileName}${this._connector
                            .sessionSuffix}`,
                    })
                    .then(response => {
                        let respObject = JSON.parse(response[0]);
            ...
            describe({ fileName = null, startFrom = null, take = null, orderBy = null }) {
                ...
                return utils
                    .ajaxp({
                        url: `${this._url}describe?${paramsString
        ...
        ServiceContainer.register(
            serviceInfo => serviceInfo.serviceType == 'StaticStorage',
            StaticSourceService,
        );
...
sGis.module(
    'sp.services.TileService',
    ['sp.services.MapService', 'TileLayer', 'TileScheme', 'sp.services.ServiceContainer'],
    (MapService, TileLayer, TileScheme, ServiceContainer) => {
        ...
        class TileService extends MapService {
            ...
            _setLayer() {
                ...
                this._layer = new TileLayer(this._getUrl(), {
            ...
            _getUrl() {
                if (this.serviceInfo.sourceUrl) {
                    ...
                    return (
                        this.url +
                        'tile/{z}/{y}/{x}' +
                        (this.connector.sessionId ? '?_sb=' + this.connector.sessionId : '')
                    );
                }
            }
        }
